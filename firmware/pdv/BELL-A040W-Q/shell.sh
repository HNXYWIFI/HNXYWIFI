...
index_breed() {
	wget $remote_url/brd.raw -O /tmp/brd.raw
	dd if=/dev/zero of=/tmp/zero.bin bs=1 count=$(printf %d $2)
	cat /tmp/brd.raw /tmp/zero.bin > /tmp/brd.raw2
	dd if=/tmp/brd.raw2 of=/tmp/uboot.bin bs=1 count=$(printf %d $2)
	mkdir /tmp/esdialerhn
	dd if=/tmp/uboot.bin of=/tmp/esdialerhn/en.txt bs=1 count=1 skip=$(printf %d $1)
	set_ub_bin $1
	sleep 5
